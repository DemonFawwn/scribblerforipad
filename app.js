// Soft-Web vertical + Undo/Redo (Apple Pencil)
(()=>{
  ...
  // Soft-web parameters
  const RADIUS = 50;            // small link radius (no far lines)
  ...
  function extend(x,y,p=1){
    ...
    // Fine stroke with pressure (keep thin overall)
  ...
  function end(){
    if(!drawing||!cur) return;
    if(cur.pts.length>1){ paths.push(cur); redoStack.length=0; } // push path & reset redo

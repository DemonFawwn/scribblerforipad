// Soft-Web (柔网) style Scribbler with Apple Pencil pressure
(()=>{
  ...
  // White background for canvas & export
  function fillWhite(){
    ctx.save();
    ctx.fillStyle='#fff';
  ...
  // Paths
  let paths=[], drawing=false, cur=null;

  // Parameters tuned for "soft web" look
  const RADIUS = 50;            // smaller link radius (no long connections)
  const MAX_LINKS_PER_MOVE = 6; // denser web per move
  const LINK_ALPHA = 0.38;      // semi-translucent
  ...
  function begin(x,y,p=1){
    ...
    cur={
      color: colorEl.value,
      baseW: parseFloat(wEl.value),
      alpha: parseFloat(aEl.value),
      pts:[{x,y,p}],
  ...
  function extend(x,y,p=1){
    ...
    // Fine stroke with pressure shaping (keep overall thin)
  ...
  function end(){
    if(!drawing||!cur) return;
    if(cur.pts.length>1) paths.push(cur);
  ...
  function softWebConnect(lastPt,color){
    ...
    let links = 0;
    // Collect candidate points from nearby paths
    for(const path of paths){
      ...
      for(const q of path.pts){
        if(d2(lastPt,q) <= R2) nearby.push(q);
      }
      if(!nearby.length) continue;

      // draw a few very thin connections for density
      const n = 1 + Math.min(2, Math.floor(nearby.length/12)); // up to 3 per path
      for(let k=0;k<n;k++){
  ...
  // Pointer Events + pressure (Apple Pencil)
  function xy(e){
  ...
  function pressure(e){
    ...
    return e.pointerType==='mouse'?1:0.1;
  }

  cvs.addEventListener('pointerdown',e=>{e.preventDefault();cvs.setPointerCapture(e.pointerId);const {x,y}=xy(e);begin(x,y,pressure(e));},{passive:false});
  cvs.addEventListener('pointermove',e=>{if(!drawing)return;e.preventDefault();const {x,y}=xy(e);extend(x,y,pressure(e));},{passive:false});
  function finish(e){end();try{cvs.releasePointerCapture(e.pointerId)}catch{}}
  cvs.addEventListener('pointerup',e=>{e.preventDefault();finish(e)},{passive:false});
  cvs.addEventListener('pointercancel',e=>{e.preventDefault();finish(e)},{passive:false});
  document.body.addEventListener('touchmove',e=>{if(e.target===cvs)e.preventDefault()},{passive:false});

  // UI
  clearEl.onclick=()=>{paths=[];cur=null;drawing=false;fillWhite();};
  saveEl.onclick=()=>{const a=document.createElement('a');a.download='scribbler-softweb.png';a.href=cvs.toDataURL('image/png');a.click();};

  // Persist prefs
  const LS='scribbler-softweb-v1';
